...
public class BusinessDateWritePlatformServiceImpl implements BusinessDateWritePlatformService {
    ...
    private final BusinessDateUpdateRequestMapper mapper;

    @Override
    public BusinessDateResponse updateBusinessDate(BusinessDateUpdateRequest request) {
        BusinessDateResponse response = mapper.map(request);
    ...
    public void increaseDateByTypeByOneDay(BusinessDateType businessDateType) throws JobExecutionException {
        var businessDateEntity = repository.findByType(businessDateType);
        List<Throwable> exceptions = new ArrayList<>();

        var businessDate = businessDateEntity.map(BusinessDate::getDate).orElse(DateUtils.getLocalDateOfTenant());
        businessDate = businessDate.plusDays(1);
        try {
            var response = BusinessDateResponse.builder().type(businessDateType).description(businessDateType.getDescription())
    ...
    private void adjustDate(BusinessDateResponse response) {
        ...
        if (isCOBDateAdjustmentEnabled && BusinessDateType.BUSINESS_DATE.equals(response.getType())) {
            var res = BusinessDateResponse.builder().type(BusinessDateType.COB_DATE).description(BusinessDateType.COB_DATE.getDescription())
                    .date(response.getDate().minusDays(1)).build();
            updateOrCreateBusinessDate(res);
            response.addAllChanges(res.getChanges());
        }
    }

    private void updateOrCreateBusinessDate(BusinessDateResponse response) {
        BusinessDateType businessDateType = response.getType();
        Optional<BusinessDate> businessDate = repository.findByType(businessDateType);

        if (businessDate.isEmpty()) {
            var newBusinessDate = BusinessDate.instance(businessDateType, response.getDate());
            repository.save(newBusinessDate);
            response.addChange(businessDateType, newBusinessDate.getDate());
        } else {
            updateBusinessDate(businessDate.get(), response);
        }
    }

    private void updateBusinessDate(BusinessDate businessDate, BusinessDateResponse response) {
        if (DateUtils.isEqual(businessDate.getDate(), response.getDate())) {
            return;
        }

        businessDate.setDate(response.getDate());
        repository.save(businessDate);

        response.addChange(businessDate.getType(), response.getDate());
    }
}
